...
import io.runebox.kasm.ir.conversion.asm2stack.AsmUtil;
...
public class AsmInsnReader {
    ...
    private void convertTypeInsnNode(TypeInsnNode insn) {
        switch (insn.getOpcode()) {
            case Opcodes.NEW:
                iv.visitNew(io.runebox.kasm.ir.conversion.asm2stack.AsmUtil.pathFromInternalName(insn.desc));
                break;

            case Opcodes.ANEWARRAY:
                RefType baseType = io.runebox.kasm.ir.conversion.asm2stack.AsmUtil.refTypeFromInternalName(insn.desc);
                iv.visitNewArray(new ArrayType(baseType, 1), 1);
                break;

            case Opcodes.CHECKCAST:
                iv.visitReferenceCast(io.runebox.kasm.ir.conversion.asm2stack.AsmUtil.refTypeFromInternalName(insn.desc));
                break;

            case Opcodes.INSTANCEOF:
                iv.visitInstanceOf(io.runebox.kasm.ir.conversion.asm2stack.AsmUtil.refTypeFromInternalName(insn.desc));
                break;
    ...
    private void convertFieldInsnNode(FieldInsnNode insn) {
        Path fieldOwner = io.runebox.kasm.ir.conversion.asm2stack.AsmUtil.pathFromInternalName(insn.owner);
        Type fieldType = io.runebox.kasm.ir.conversion.asm2stack.AsmUtil.fromDescriptor(insn.desc);
    ...
    private void convertMethodInsnNode(MethodInsnNode insn) {
        Path owner = io.runebox.kasm.ir.conversion.asm2stack.AsmUtil.pathFromInternalName(insn.owner);
        MethodDescriptor desc = io.runebox.kasm.ir.conversion.asm2stack.AsmUtil.parseMethodDescriptor(insn.desc);
    ...
    private void convertInvokeDynamicInsnNode(InvokeDynamicInsnNode insn) {
        MethodDescriptor descriptor = io.runebox.kasm.ir.conversion.asm2stack.AsmUtil.parseMethodDescriptor(insn.desc);
        ...
        if (value instanceof Integer) {
            ...
            switch (asmType.getSort()) {
                case org.objectweb.asm.Type.METHOD:
                    MethodDescriptor desc = io.runebox.kasm.ir.conversion.asm2stack.AsmUtil.parseMethodDescriptor(asmType);
                    return new MethodTypeConstant(desc.getParameterTypes(), desc.getReturnType());

                case org.objectweb.asm.Type.OBJECT:
                case org.objectweb.asm.Type.ARRAY:
                    return new ClassConstant((RefType) io.runebox.kasm.ir.conversion.asm2stack.AsmUtil.fromAsmType(asmType));
    ...
    private PushableConstant convertLdcValue(Object value) {
        if (value instanceof Integer) {
            ...
            Type type = io.runebox.kasm.ir.conversion.asm2stack.AsmUtil.fromDescriptor(constantDynamic.getDescriptor());
    ...
    private Handle convertMethodHandle(org.objectweb.asm.Handle handle) {
        Path owner = io.runebox.kasm.ir.conversion.asm2stack.AsmUtil.pathFromInternalName(handle.getOwner());
        ...
        switch (tag) {
            ...
            case Opcodes.H_PUTSTATIC:
                FieldRef fieldRef = new FieldRef(owner, handle.getName(), io.runebox.kasm.ir.conversion.asm2stack.AsmUtil.fromDescriptor(handle.getDesc()));
            ...
            case Opcodes.H_INVOKEINTERFACE:
                MethodDescriptor desc = io.runebox.kasm.ir.conversion.asm2stack.AsmUtil.parseMethodDescriptor(handle.getDesc());
